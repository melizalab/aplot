...
#define CreateMenuToggle(widget, parent, name, label, set, userdata) \
	widget = XtVaCreateManagedWidget(name, xmToggleButtonWidgetClass, parent,	\
		...
		NULL);																	\
	XtAddCallback(widget, XmNvalueChangedCallback, sono_popupmenu_cb, (XtPointer)plot);	\
	XmStringFree(xmstr1);
...
#define CreateMenuButton(widget, parent, name, label, userdata) \
	widget = XtVaCreateManagedWidget(name, xmPushButtonWidgetClass, parent,	\
		...
		XmNuserData, userdata,													\
		NULL);																	\
	XtAddCallback(widget, XmNactivateCallback, sono_popupmenu_cb, (XtPointer)plot);	\
	XmStringFree(xmstr1);
 ...
 * PROTOTYPES
 **********************************************************************************/
int sono_open(PLOT *plot);
int sono_display(PLOT *plot);
int sono_set(PLOT *plot);
int sono_close(PLOT *plot);
int sono_print(PLOT *plot, FILE *fp, char **ret_filename_p);
int sono_save(PLOT *plot);
int sono_play(PLOT *plot, int reqoffset, int reqsize, short **retsamples, int *retsize, float *playtime);
static void sono_expose_callback(Widget w, XtPointer clientData, XtPointer callData);
static void sono_resize_callback(Widget w, XtPointer clientData, XtPointer callData);
static void sono_event_handler(Widget w, XtPointer clientData, XEvent *event, Boolean *b);
static void sono_playmarker(PLOT *plot, float t);
static void sono_clearmarks(PLOT *plot);
static void sono_drawstartmark(PLOT *plot, float t);
static void sono_drawstopmark(PLOT *plot, float t);
static void sono_popupmenu_cb(Widget w, XtPointer clientData, XtPointer callData);
static void ASCII85_output(FILE *fp, int unsigned long word, int count);
static void DrawSono(Display *dpy, Pixmap pixmap, GC gc, unsigned long *colors, int ncolors, int depth,
...
static void PSDrawSono(FILE *fp, int ncolors,
	int offx, int offy, int offx2, int offy2, int width, int height,
	struct sonogram *sono, short *samples, int nsamples,
	float xrangemin, float xrangemax,
	int xmin, int xmax, float gsmin, float gsmax, float fmin, float fmax);
static void PSDrawSono_Color(FILE *fp, int sonocolormap, 
	int offx, int offy, int offx2, int offy2, int width, int height,
	struct sonogram *sono, short *samples, int nsamples,
	float xrangemin, float xrangemax,
	int xmin, int xmax, float gsmin, float gsmax, float fmin, float fmax);

float sono_conv_pixel_to_time(PLOT *plot, int x);
float sono_conv_pixel_to_val(PLOT *plot, int y);
...
int sono_open(PLOT *plot)
{
	struct sonodata *plotdata;
	Display *dpy;
	Screen *scr;
	Widget w, mw, m0, m1, m2, m3, m4, m5, m8, m9;
	Widget m6, m6a, m6a1, m6a2, m6a3, m6a4, m6a5, m6a6, m6a7, m6a8, m6a9, m6a10, m6a11;
	...
	plot->plotdata = (void *)calloc(1, sizeof(struct sonodata));
	plotdata = (struct sonodata *)plot->plotdata;
	...
	plot->plot_widget = XtVaCreateManagedWidget("sono", xmDrawingAreaWidgetClass, plot->panel->panel_container,
		XmNheight, defaults.sono_height,
		XmNwidth, defaults.width,
		XmNmarginHeight, 0,
		XmNmarginWidth, 0,
		NULL);
	XtAddCallback(plot->plot_widget, XmNexposeCallback, sono_expose_callback, (XtPointer)plot);
	XtAddCallback(plot->plot_widget, XmNresizeCallback, sono_resize_callback, (XtPointer)plot);
	...
	*/
	...
	plot->drawxaxis = defaults.sono_drawxaxis;
	plot->drawyaxis = defaults.sono_drawyaxis;
	...
	*/
	...
	CreateMenuToggle(m6a7, m6a, "fftsize4096", "4096", (plot->group->fftsize == 4096), "FFT size 4096");
	CreateMenuToggle(m6a8, m6a, "fftsize8192", "8192", (plot->group->fftsize == 8192), "FFT size 8192");
	CreateMenuToggle(m6a9, m6a, "fftsize16384", "16384", (plot->group->fftsize == 16384), "FFT size 16384");
	CreateMenuToggle(m6a10, m6a, "fftsize32768", "32768", (plot->group->fftsize == 32768), "FFT size 32768");
	CreateMenuToggle(m6a11, m6a, "fftsize65536", "65536", (plot->group->fftsize == 65536), "FFT size 65536");
	...
	*/
	XtAddEventHandler(w, KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | Button1MotionMask | Button2MotionMask,
		FALSE, (XtEventHandler) sono_event_handler, (XtPointer)plot);

	plot->plot_playmarker = sono_playmarker;

	plot->plot_clearmarks = sono_clearmarks;
	plot->plot_drawstartmark = sono_drawstartmark;
	plot->plot_drawstopmark = sono_drawstopmark;
	plot->plot_conv_pixel_to_time = sono_conv_pixel_to_time;

	plot->plot_display = sono_display;
	plot->plot_set = sono_set;
	plot->plot_close = sono_close;
	plot->plot_print = sono_print;

	plot->plot_save = sono_save;

	plot->plot_event_handler = sono_event_handler;
	plot->plot_play = sono_play;
...
int sono_display(PLOT *plot)
{
	struct sonodata *plotdata = (struct sonodata *)plot->plotdata;
...
int sono_set(PLOT *plot)
{
	...
	** it isn't realized during the call to sono_open().
	*/
	...
	*/
	DrawSono(dpy, plotdata->pixmap, plotdata->drawing_GC, plotdata->colors,
...
int sono_print(PLOT *plot, FILE *fp, char **ret_filename_p)
{
	...
	** 3) add the extension: _sono_<entry>_<startms>_<stopms>.eps
	...
	*/
	...
	if (*ch == '.') *ch = '\0';
	sprintf(tempstr, "_sono_%ld_%d_%d.eps", plot->group->entry, (int)plot->group->xrangemin, (int)plot->group->xrangemax);
	...
	*/
	...
	if (fp != NULL)
	{
		...
		if (plotdata->colormap == CMAP_GRAY)
		{
			PSDrawSono(fp, 256, offx, offy, offx2, offy2, width, height,
				plotdata->sono, plot->group->samples, plot->group->nsamples,
				plot->group->xrangemin, plot->group->xrangemax,
				xmin, xmax, gsmin, gsmax, fmin, fmax);
		}
		...
		{
			PSDrawSono_Color(fp, plotdata->colormap, offx, offy, offx2, offy2, width, height,
				plotdata->sono, plot->group->samples, plot->group->nsamples,
				plot->group->xrangemin, plot->group->xrangemax,
				xmin, xmax, gsmin, gsmax, fmin, fmax);
		}
...
int sono_save(PLOT *plot)
{
	...
	{
		tmin = sono_conv_pixel_to_time(plot, plot->markx1);
		tmax = sono_conv_pixel_to_time(plot, plot->markx2);
		if (tmin > tmax)
		{
			ttmp = tmax;
			tmax = tmin;
	...
	*/
	...
	while ((ch > filename) && (*ch != '.')) ch--;
	if (*ch == '.') *ch = '\0';
	sprintf(tempstr, "_sono_%ld_%d_%d", plot->group->entry, (int) tmin, (int) tmax);
	strcat(filename, tempstr);
	ch = &(filename[strlen(filename)-1]);
	while ((ch > filename) && (*ch != '/')) ch--;
	if (*ch == '/') filename = ++ch;
	...
	if (resource_data.outputseq2 == 1)
	{
		...
		if ((pcmfp = pcm_open(filename, "w")) != NULL)
		{
			...
			if (pcm_write(pcmfp, samples + start, stop - start + 1) != -1)
				printf("SONO: saved %d samples into '%s'\n", stop-start+1, filename);
			else
				fprintf(stderr, "SONO: Error writing to '%s'\n", filename);
			pcm_close(pcmfp);
		}
		else fprintf(stderr, "SONO: Error opening '%s'\n", filename);
	}
	...
	else
	{
		...
		if ((fp = fopen(filename, "w")) != NULL)
		{
			...
			printf("SONO: saved %ld samples into '%s'\n", numwritten, filename);
			fclose(fp);
		}
		else fprintf(stderr, "SONO: Error opening '%s'\n", filename);
	}
			...
			else
			{
				...
				if ((keybuf = XKeysymToString(key_sym)) != NULL)
				{
					if ((keyevent->state == ControlMask) &&  (keybuf[0] == 'x'))
					{
						...
						defaults.sono_drawxaxis = plot->drawxaxis;
					...
					else if ((keyevent->state == ControlMask) && (keybuf[0] == 'y'))
					{
						...
						defaults.sono_drawyaxis = plot->drawyaxis;
						XtVaSetValues(XtNameToWidget(plot->plot_popupmenu_widget, "yaxis"), XmNset, plot->drawyaxis, NULL);
						sono_set(plot);
					}
					else if ((keyevent->state == (ControlMask|ShiftMask)) && (!strcmp(keybuf, "x")))
					{
						plotdata->drawxgrid = !(plotdata->drawxgrid);
						sono_set(plot);
					}
					else if ((keyevent->state == (ControlMask|ShiftMask)) && (!strcmp(keybuf, "y")))
					{
						plotdata->drawygrid = !(plotdata->drawygrid);
						sono_set(plot);
					}
					else if ((keyevent->state != ShiftMask) && (!strcmp(keybuf, "equal")))
					{
						ui_get_gs_range(&gsmin, &gsmax);
						gsmax -= 5;
						ui_set_gs_range(gsmin, gsmax);
						plot->group->gsmin = gsmin;
						plot->group->gsmax = gsmax;
						sono_set(plot);
					}
					else if ((keyevent->state != ShiftMask) && (!strcmp(keybuf, "minus")))
					{
						ui_get_gs_range(&gsmin, &gsmax);
						gsmin -= 5;
						ui_set_gs_range(gsmin, gsmax);
		...
		case ButtonPress:
			...
			switch (butevent->button)
			{
				case Button1:
					...
					{
						char title[256];
						sprintf(title, "%.2f", sono_conv_pixel_to_time(plot, butevent->x));
						if (IS_Y_INBOUNDS(plot, butevent->y))
							sprintf(title + strlen(title), " (%.0f)", sono_conv_pixel_to_val(plot, butevent->y));
				...
				case Button2:
					...
					panel_drawstartmark(plot->panel, sono_conv_pixel_to_time(plot, butevent->x));

					if (IS_X_INBOUNDS(plot, butevent->x))
					{
						char title[256];
						sprintf(title, "%.2f", sono_conv_pixel_to_time(plot, butevent->x));
						sprintf(title + strlen(title), " : %.2f (0)", sono_conv_pixel_to_time(plot, butevent->x));
						XtVaSetValues(plot->panel->panel_shell, XmNtitle, title, NULL);
					}
					break;
		...
		case ButtonRelease:
			...
			switch (butevent->button)
			{
				...
				case Button2:
					...
					panel_drawstopmark(plot->panel, sono_conv_pixel_to_time(plot, x));
			...
			else if (motevent->state & Button2Mask)
			{
				if (plot->markx1 != -1)
				{
					...
					if (IS_X_INBOUNDS(plot, motevent->x))
					{
						char title[256];
						float t1, t2;
						t1 = sono_conv_pixel_to_time(plot, MIN(plot->markx1, motevent->x));
						t2 = sono_conv_pixel_to_time(plot, MAX(plot->markx1, motevent->x));
...
static void sono_popupmenu_cb(Widget w, XtPointer clientData, XtPointer callData)
{
	...
	if (!strcmp(idstring, "X axis"))
	{
		...
		defaults.sono_drawxaxis = drawxaxis;
		sono_set(plot);
	}
...
** TIME, VAL are in milliseconds and frequency (hertz) units, respectively.
	...
	int xmin, int xmax, float gsmin, float gsmax, float fmin, float fmax)
{
	...
	** When I select 2 files and draw a sono for each, then the second one
	...
	int xmin, int xmax, float gsmin, float gsmax, float fmin, float fmax)
{
	...
	*/
	if ((sono->method == METHOD_FFT) || (sono->method == METHOD_FFTW) || (sono->method == METHOD_MULTITAPER))
	{
		tmin = xmin / (((100.0 - overlap) / 100.0) * fftsize);
		tmax = xmax / (((100.0 - overlap) / 100.0) * fftsize);
		if (tmax >= ntimebins) tmax = ntimebins - 1;
	...
	int xmin, int xmax, float gsmin, float gsmax, float fmin, float fmax)
{
	...
	ncolors = 256;
	cvals = (unsigned long *)calloc(ncolors, sizeof(unsigned long));
	(*((colormap[sonocolormap]).cmap))(NULL, ncolors, NULL, cvals);
	...
	*/
	if ((sono->method == METHOD_FFT) || (sono->method == METHOD_FFTW) || (sono->method == METHOD_MULTITAPER))
	{
		...
		tmax = xmax / (((100.0 - overlap) / 100.0) * fftsize);
		if (tmax >= ntimebins) tmax = ntimebins - 1;
		multfactor = (ncolors - 1.0) / (gsmax - gsmin);
		...
		for (y=0; y < nfreqbins; y++)
		{
			for (t=tmin; t <= tmax; t++)
			{
				...
				for (component = 0; component < 3; component++)
				{
					switch (component)
					{
						case 0: buf[j%4] = (int)(cvals[colorindex] & 0xff0000) >> 16; break;
						case 1: buf[j%4] = (int)(cvals[colorindex] & 0xff00) >> 8; break;
						case 2: buf[j%4] = (int)(cvals[colorindex] & 0xff); break;
					}
					if (((++j)%4) == 0)
					{
						word = ((unsigned long)(((unsigned int)buf[0] << 8) + buf[1]) << 16) + (((unsigned int)buf[2] << 8) + buf[3]);
